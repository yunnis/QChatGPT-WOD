...
from plugins.base.main import  groups, get_hero_id
from plugins.dungeon.main import  get_hero_names
import plugins.base.jscode as base
import plugins.base.driver as driver
import plugins.base.pqsldb as db
import plugins.report.sqlstatement as sql
...
class MyPlugin(BasePlugin):
    ...
    def _get_list(self, group, count):
        ...
        # 加载5个战报, tr[1] 是头
        # if not d.wait('//*[@id="main_content"]/div/form/table/tbody/tr[6]/td[1]/span'):
            # return False, f"获取战报列表 失败, 超时 或者 没有战报"
            ...
            for row in table.find_all('tr')[1:]:  # Skip the header row
                ...
                # report_table.add_row([value, date, dungeon])
    ...
    def __find_report(self, group, report_id):
        ...
        if resp:        # 加载战报, tr[1] 是头 d.wait('//*[@id="main_content"]/div/form/table/tbody/tr[1]/td[1]/span'):
            ...
            for row in table.find_all('tr')[1:]:  # Skip the header row
                ...
                if report_id is None:
                    is_find = True
                    break
                if value and value == report_id:
                    is_find = True
                    break
                
            if is_find:
                self.ap.logger.critical(f'find report {report_id}, num is {num}, name {dungeon}, time {date}')
    ...
    def __pares_db_pass(self, report_id):
        records = self.__db.exec_all(sql.select_battle_report, (report_id,)) # execute 就会开启事务, 要不用with 之类自动提交, 要不手动提交
        self.__db.commit()
        if records:
    ...
    # 换为psql tz时间戳
    def __wod_time_to_tstz(self, wod_time : str):
    ...
    def _parse_pass(self, group, report_id):
        print(f'解析战报 {group}, {report_id}')
        #  先去找id, 再去db找, db 没有就读
        if report_id is not None:
            ret, ret_str = self.__pares_db_pass(report_id)
            if ret:
                return True, ret_str

        ret = None
        ret_str = ""
        have_report, have_report_str, num, id, dungeon, dungeon_time = self.__find_report(group, report_id)
        if id is not None: # 赋值是因为支持了report_id 为空
            ret, ret_str = self.__pares_db_pass(id)
        ...
        if not have_report:
            self.ap.logger.error(have_report_str)
        ...
        # <a href="../hero/profile.php?id=0" onclick="return wo('../hero/profile.php?id=0);">name</a>
        ...
        dungeon_time_tz, time_str = self.__wod_time_to_tstz(dungeon_time)
        if dungeon_time_tz is None:
            return False, time_str 
        # insert_player = "["+",".join(f'({p_id}, {p_name}, {p_layer})' for p_id, p_name, p_layer in players)+"]"
        self.__db.exec_no_fetch(sql.insert_battle_report, (id, timestamp, dungeon, dungeon_time_tz, max_layer, players,))
        self.__db.exec_no_fetch('commit')
        return True, output
    ...
    def __parse_buff(self, group, report_id):
        ...
        records = self.__db.exec_all(sql.select_battle_report, (id,)) # execute 就会开启事务, 要不用with 之类自动提交, 要不手动提交
        self.__db.commit()
        d = driver.WodDriver.get_instance()
        if not records:
            self._parse_pass(group, id)
            hero_id = get_hero_id(str(group))
            url = f'http://delta.world-of-dungeons.org/wod/spiel/dungeon/report.php?session_hero_id={hero_id}'
            d.open(url)
        ...
        self.ap.logger.critical(f'{report_id} input {len(inputs)}')
        layer = 0
        is_db_tran = False
        ...
        for input in inputs:
            if layer > 1:
                ...
                if next_layer is None:
                    self.ap.logger(f'buff 找下一层的按钮失败了, 当前{layer}. {id}')
                    continue
                next_layer.click()
                                    ...
                                    if ( hero_link and hero_link.has_attr('href') ):
                                        ...
                                        if span:
                                            ...
                                            if match:
                                                ...
                                                if hero_id not in grouped_data[round_num]:
                                                    grouped_data[round_num][hero_id] = {
                                                    'number': number,
                                                    'hero_name': hero_name,
                                                    #'hero_id': hero_id,
                                                    'content': {}
                                                ...
                                                # mouse_over_contents.append((number, round_headline.text.strip(), hero_name, hero_id, actual_content))
                        # break  # 跳出循环，因为已经找到要处理的部分
                    next_element = next_element.find_next_sibling()
                # 回合表太冗余了, 如果统计一层的数据, 要查多个记录再汇总
                # self.__db.exec_one(sql.insert_report_round, (create_at, round_num, grouped_data,))
            if round_num > 0:
                print(f'insert report_layer, this layer {layer}, {round_num}')
                if self.__db.exec_no_fetch(sql.insert_report_layer, (create_at, id, layer, round_count, json.dumps(grouped_data))) is None:
    ...
    # 外部输入: 战报id, 层, 回合, 名字
    def __get_buff(self, group, report_id, layer, turn, name, skill, prop):
        global layer_reports
        if report_id not in layer_reports:
            records = self.__db.exec_one(sql.select_report_layer, (report_id, layer,))
            self.__db.commit()
            if records is None:
        ...
        if records is None:
            return False, f'战报 {report_id} 还未执行过 解析buff'
        hero_names = get_hero_names(group)
        if hero_names is None or name not in hero_names:
            return False, f'传入 {name} 不在缓存中, 缓存长度 {len(hero_names)}'
        h_id = hero_names[name]
        ...
        for round_num, heroes in detail.items():
            ...
            if round_num != turn:
                continue
            if h_id not in heroes:
                return False, f'{report_id} 中找不到{h_id}-{name}'
            heroes = heroes[h_id]
    ...
    def doReport(self, ctx):
        ...
        msgs = msg.split()
        print(f'doReport Test input split : \  {msgs}\n')

        # 消息处理逻辑
        command_map = {
        "战报列表": lambda: self._get_list(group, 5),
        "上个地城": lambda: self._get_list(group, 1),
        "完成率": lambda: self._parse_pass(group, msgs[1] if len(msgs) > 1 else None),
        "解析buff": lambda: self.__parse_buff(group, msgs[1] if len(msgs) > 1 else None),
        "buff": lambda: self.__get_buff(group, msgs[1], msgs[2], msgs[3], msgs[4], msgs[5], None)
        }

        if msgs[0] in command_map:
                return command_map[msgs[0]]()
        # try:
        #     if msgs[0] in command_map:
        #         print(msgs[0])
        #         return command_map[msgs[0]]()
        # except Exception as e:
        #     return False, f'{msg} 出错了, {e}'
